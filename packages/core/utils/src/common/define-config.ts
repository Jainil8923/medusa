...
export function defineConfig(
  config: InputConfig = {},
  options: DefineConfigOptions = { isProduction: false }
): ConfigModule {
...
function resolveModules(
  ...
  { isProduction, isCloud }: DefineConfigOptions & { isCloud?: boolean }
): Exclude<ConfigModule["modules"], undefined> {
  const sharedModules = [
   ...
   */
  const modules: InputConfig["modules"] = isProduction
    ? prodModules
    : devModules
...
function normalizeProjectConfig(
  ...
  { isProduction }: DefineConfigOptions
